...
describe("Data Lake Tables standardization", () => {
    ...
    test("uses the Data Lake Tables identity", () => {
        expect(String(pbiviz.visual.name)).toMatch(/dataLakeTables|data-lake-tables/i);
        expect(String(pbiviz.visual.displayName)).toMatch(/Data Lake Tables/i);
        expect(renderer).toContain("DATA LAKE TABLES");
    });
